...
char* FCGI_getenv(const char *param)
{
	char** p;
	...
	for (p = g_environ; *p != NULL; p++)
	{
		char* equal = strchr(*p, '=');
		if (equal != NULL && memcmp(param, *p, equal - *p) == 0)
		{
			return equal + 1;
		}
		else
...
size_t FCGI_fwrite(void *ptr, size_t size, size_t nmemb, FCGI_FILE *fp)
{
    ...
    if(fp->stdio_stream)
        ...
        n = FCGX_PutStr((char *) ptr, (int)(size * nmemb), fp->fcgx_stream);
        return (n/size);
    }
    return (size_t)EOF;
}
